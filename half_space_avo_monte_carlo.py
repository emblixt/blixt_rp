...
import ElasticEquations as myee 
import crossplot as myxp
import curve_fitting as mycf
import plot_reflectivity as mypr
...
def plot_multi_interfaces(sums, intfs, fbase=None, suffix=None):
    """
    :param sums: 
        dict
    ...
    ax1.plot([0, 0], [-0.75, 0.75], 'k--', lw=0.5, label='_nolegend_')
    ax1.plot([-0.75, 0.75], [0, 0], 'k--', lw=0.5, label='_nolegend_')
...
def plot_one_interface(sums, name1, name2, color, fig_ig, ax_ig, fig_refl, ax_refl, n_samps=1000):
    ...
    for i, params in enumerate(zip(vp1, vp2, vs1, vs2, rho1, rho2)):
        refs[i, :] = myee.reflectivity(*params)(theta)

    refl_stds = np.std(refs, 0)

    # Calculate the mean reflectivity curve
    mean_refl = myee.reflectivity(
        sums[name1]['VpMean'],
        sums[name2]['VpMean'],
        sums[name1]['VsMean'],
        sums[name2]['VsMean'],
    ...
    intercept = myee.intercept(vp1, vp2, rho1, rho2)
    gradient = myee.gradient(vp1, vp2, vs1, vs2, rho1, rho2)
